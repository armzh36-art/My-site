...
function isSpammyEmail(email) {
  ...
  if (local.length < 3 || local.length > 25) return true;
  if (/(.)\1{5,}/.test(local)) return true;
  if (!/[aeiou]/i.test(local)) return true;
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/register" && request.method === "POST") {
      ...
      // 🔐 EMAIL CHECK (NEW PART)
      if (!isValidEmail(email)) {
        return json({ error: "Invalid email format" }, 400);
      }

      if (isDisposable(email)) {
        return json({ error: "Temporary emails not allowed" }, 400);
      }

      if (isSpammyEmail(email)) {
        return json({ error: "Suspicious email detected" }, 400);
      }
